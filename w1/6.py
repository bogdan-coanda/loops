...
if __name__ == "__main__":
	...
	diagram = Diagram(6)
	headCycle = diagram.cycleByAddress['0000']
	
	startTime = time()
	step()
	
	diagram.cycleByAddress['0000'].chain.connect(2)
	rv = connectSingles(diagram)	
	revertSingles(rv)
	diagram.cycleByAddress['0000'].chain.revert()	
	'''

	diagram.cycleByAddress['0001'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0002'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0002'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0010'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0011'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0012'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0020'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0021'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0022'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['0031'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['1134'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['1130'].chain.connect(2)
	connectSingles(diagram)
	
	diagram.cycleByAddress['1131'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['1121'].chain.connect(3)
	connectSingles(diagram)

	diagram.cycleByAddress['1213'].chain.connect(2)
	connectSingles(diagram)

	diagram.cycleByAddress['1214'].chain.connect(2)
	connectSingles(diagram)			

	diagram.cycleByAddress['1210'].chain.connect(2)
	connectSingles(diagram)			

	diagram.cycleByAddress['1211'].chain.connect(3)
	connectSingles(diagram)			

	diagram.cycleByAddress['1033'].chain.connect(3)
	connectSingles(diagram)			

	diagram.cycleByAddress['1021'].chain.connect(2)
	connectSingles(diagram)			

	# diagram.cycleByAddress['0114'].chain.connect(3)
	# connectSingles(diagram)			

	# diagram.cycleByAddress['0124'].chain.connect(3)
	# connectSingles(diagram)			


	# diagram.cycleByAddress['1201'].chain.connect(3)
	# connectSingles(diagram)			
	# 

	# 
	# 
	# diagram.cycleByAddress['1203'].chain.connect(3)
	# connectSingles(diagram)			
	'''																														
	show(diagram)
