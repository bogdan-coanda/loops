...
class MX (object):
	...
	def filter_avtuples(self, prev_avtuples=None):
		return [t for t in (prev_avtuples or self.diagram.loop_tuples)
			if len([loop for loop in t if not loop.available and not loop.extended]) == 0]	
	...
	def find_min_matched_tuples(self, unicycle_chains, avtuples, next_sample_lengths=defaultdict(lambda:1)):
		...
		for chain in unicycle_chains:
			...
			curr_cycle_tuples = [node.loop.tuple for node in cycle.nodes if node.loop.available or node.loop.extended]
