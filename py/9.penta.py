...
if __name__ == "__main__":
	...
	def isStar5Availabled(node):
		for nln in node.loop.nodes:
			...
			if curr.loopBrethren[4].cycle.moved:
				return False
			if curr.loopBrethren[5].cycle.moved:
				return False				
		return True
	...
	# 5by6 [~][!]
	node = diagram.nodeByAddress['00000008']
	...
	for i,nln in enumerate(node.loop.nodes):
		...
		moveCycle(curr.loopBrethren[3].cycle, 4, (i*(diagram.spClass-2)+4)/((diagram.spClass-2)*(diagram.spClass-1)), 0)#, True)
		moveCycle(curr.loopBrethren[4].cycle, 3, (i*(diagram.spClass-2)+5)/((diagram.spClass-2)*(diagram.spClass-1)), 0)#, True)
		moveCycle(curr.loopBrethren[5].cycle, 2, (i*(diagram.spClass-2)+6)/((diagram.spClass-2)*(diagram.spClass-1)), 0)#, True)
