...
if __name__ == "__main__":
	...
	# 5by6 [~][!]
	node = diagram.nodeByAddress['000006']
	...
	for i,nln in enumerate(node.loop.nodes):
		...
		curr = nln.links[1].next
		showLinks(curr)	
		moveCycle(curr.loopBrethren[0].cycle, 2, (i*(diagram.spClass-2)+1)/30, 0)#, True)
		moveCycle(curr.loopBrethren[1].cycle, 3, (i*(diagram.spClass-2)+2)/30, 0)#, True)
		moveCycle(curr.loopBrethren[2].cycle, 3, (i*(diagram.spClass-2)+3)/30, 0)#, True)
		moveCycle(curr.loopBrethren[3].cycle, 2, (i*(diagram.spClass-2)+4)/30, 0)#, True)
