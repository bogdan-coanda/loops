...
class Column (object):
	...
	__slots__ = ['id', 'tuples', 'firstNode', 'columnLoops', 'box_addrs', 'unavailabled']
	...
	def __init__(self, id, tuples):
		...
		self.box_addrs = set([l.firstAddress()[:-3] for l in self.columnLoops])
	...
	def __repr__(self):
		return f"⟨column:{self.id}@{self.firstNode.address}(ktype:{self.firstNode.ktype})|t:{len(self.tuples)}|cL:{len(self.columnLoops)}|bb:{len(self.box_addrs)}⟩"
