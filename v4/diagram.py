...
if __name__ == "__main__":
	...
	def next(lvl, road, path = []):
		global bcc, ncc, min, fcc, sols_superperms, FULLY_CHAINED
		bcc += 1
	...
	def back(lvl = 0, road = []):
		...
		if not FULLY_CHAINED:
			...
			sorted_chains = sorted([chain for chain in diagram.chains if len(chain.cycles) is 1], key = lambda chain: len(chain.avloops))
			min_chain = sorted_chains[0] if len(sorted_chains) else None
			#print("min chain: " + str(min_chain) + " | avloops: " + str(len(min_chain.avloops) if min_chain else None) + " | cycles: " + str(len(min_chain.cycles) if min_chain else None))
			chloops = sorted(min_chain.avloops, key = lambda loop: loop.firstNode().address) if min_chain else None
			avnodes = sorted([[node for node in loop.nodes if node.cycle.chain is min_chain][0] for loop in chloops], key = lambda node: node.address) if min_chain else None

		#if FULLY_CHAINED:
			#if (len(road) and road[0][0] is not 1) or (len(road) > 2 and road[2][0] is not 1) or (len(road) > 3 and road[3][0] is not 1) or (len(road) > 4 and road[4][0] is not 2) or (len(road) > 7 and road[7][0] is not 1) or (len(road) > 8 and road[8][0] is not 1) or (len(road) > 9 and road[9][0] is not 1) or (len(road) > 12 and road[12][0] is not 1) or (len(road) > 23 and road[23][0] is not 1):
				#return False
			#if len(road) >= 23:
